...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    const_cols = [c.name for c in summary.columns if c.unique <= 1]
    flags["has_constant_columns"] = len(const_cols) > 0
    flags["constant_columns"] = const_cols

    estimated = int(0.05 * summary.n_rows) if summary.n_rows > 0 else 0
    high_cardinality_threshold = max(20, estimated)
    high_card_cols = [c.name for c in summary.columns if not c.is_numeric and c.unique > high_cardinality_threshold]
    ...
    id_like = {"id", "user_id", "uid", "item_id", "document_id"}
    suspicious = [c.name for c in summary.columns if c.name.lower() in id_like and c.unique < summary.n_rows]
    flags["has_suspicious_id_duplicates"] = len(suspicious) > 0
    flags["suspicious_id_columns"] = suspicious
