...
class FlagsResponse(BaseModel):
    flags: Dict[str, Any] = Field(..., description="Полный набор флагов качества из core.py")
...
@app.post(
    ...
    tags=["quality"],
    summary="Получение полного набора флагов качества из CSV"
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> FlagsResponse:
    ...
    all_flags = compute_quality_flags(summary, m_table)
